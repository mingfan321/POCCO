...
class TSPModel(nn.Module):
    ...
    def forward(self, state, route=None, return_probs=False, selected_count=None):
        ...
        if state.current_node is None:
            ...
            # shape: (batch, pomo, embedding)
            probs, moed_loss = self.decoder(encoded_last_node, ninf_mask=state.ninf_mask)
            # shape: (batch, pomo, problem)
            self.aux_loss += moed_loss
...
class TSP_Decoder(nn.Module):
    ...
    def forward(self, encoded_last_node, ninf_mask):
        ...
        ninf_mask3 = torch.cat(
            (ninf_mask, torch.zeros(ninf_mask.shape[0], ninf_mask.shape[1], 1)), dim=-1)
        out_concat = multi_head_attention(q, self.k, self.v, rank3_ninf_mask=ninf_mask3)
